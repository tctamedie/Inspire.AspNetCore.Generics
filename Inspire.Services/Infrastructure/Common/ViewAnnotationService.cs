...
namespace Inspire.Services.Infrastructure.Common
{
    ...
    public class ViewAnnotationService : IViewAnnotationService
    {
            ...
            where T : IEquatable<T>
        {
            ...
            var properties = typeof(TEntity).GetProperties().Where(t => t.GetCustomAttributes(typeof(LinkAttribute), true).Any()).Select(s => new { s.Name, DataType = s.PropertyType, Attribute = (LinkAttribute)s.GetCustomAttributes(typeof(LinkAttribute), true).First() }).ToList();
            ...
            where T : IEquatable<T>
        {
            ...
            var properties = typeof(TEntity).GetProperties().Where(t => t.GetCustomAttributes(typeof(TableColumnAttribute), true).Any()).Select(s => new { s.Name, DataType = s.PropertyType, Attribute = (TableColumnAttribute)s.GetCustomAttributes(typeof(TableColumnAttribute), true).First() }).ToList();
            foreach (var record in properties)
            {
